...
function solveLPOA!(objective::AbstractExpr, p::AssocPoly)
  if objective.head == :*
    if typeof(objective.children[2]) <: Variable
      if typeof(objective.children[1]) <: Constant
    ...
    elseif typeof(objective.children[1]) <: Variable
      if typeof(objective.children[2]) <: Constant
